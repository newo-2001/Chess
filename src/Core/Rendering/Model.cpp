...
void Model::Render() const
{
    for (size_t i = 0; i < m_meshes.size(); i++)
    {
        ...
        {
            m_textures[materialIndex]->Use(0);
        }
...
void Model::LoadMesh(aiMesh* mesh, const aiScene* scene)
{
    ...
    m_meshes.push_back(std::make_unique<Mesh>(vertices.data(), mesh->mNumVertices, indices.data(), indices.size()));
    m_meshTextureIndices.push_back(mesh->mMaterialIndex);
}

void Model::LoadMaterials(const aiScene* scene)
{
    std::shared_ptr<Texture> missingTexture = std::make_shared<Texture>("resources/textures/plain.png", true);
    ...
    for (size_t i = 0; i < scene->mNumMaterials; i++)
    {
        ...
        if (material.GetTextureCount(aiTextureType_DIFFUSE))
        {
            ...
            if (material.GetTexture(aiTextureType_DIFFUSE, 0, &path) == aiReturn_SUCCESS)
            {
                ...
                std::string texturePath = "resources/textures/" + filename;
